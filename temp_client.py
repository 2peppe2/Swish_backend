...
def test_swish():
    ...
    response = swish_client.create_payment(100, 'SEK', 'https://p3trus.se/', 'Id2', 'Test', "123456780")
...
def auth_test():
    ...
    response = requests.delete("http://127.0.0.1:5000/auth/logout", headers={"Authorization": f"Bearer {access_token}"})
    print(response)
    assert response.status_code == 200
